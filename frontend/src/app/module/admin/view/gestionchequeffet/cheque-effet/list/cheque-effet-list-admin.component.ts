...
export class ChequeEffetListAdminComponent extends AbstractListController<ChequeEffetDto, ChequeEffetCriteria, ChequeEffetAdminService>  implements OnInit {
    ...
    public initCol() {
        this.cols = [
            ...
            {field: 'beneficiaire?.id', header: 'Beneficiaire'},
        ];
    }


    public async loadBeneficiaire(){
       this.fournisseurService.findAll().subscribe(beneficiaires => this.beneficiaires = beneficiaires, error => console.log(error))
    }
   ...
   public prepareColumnExport(): void {
        this.exportData = this.items.map(e => {
            return {
                ...
                'Date emission': this.datePipe.transform(e.dateEmission , 'dd/MM/yyyy hh:mm'),
                 'Montant': e.montant ,
                'Beneficiaire': e.beneficiaire?.id ,
            }
        });
        ...
        //'Beneficiaire': this.criteria.beneficiaire?.id ? this.criteria.beneficiaire?.id : environment.emptyForExport ,
        }];
      }
}
